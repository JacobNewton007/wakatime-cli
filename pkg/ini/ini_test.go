...
func TestReadInConfig(t *testing.T) {
	v := viper.New()
	v.Set("config", "testdata/zeedas.cfg")
...
func TestFilePath(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"default": {
			Expected: filepath.Join(home, ".zeedas.cfg"),
		},
		"viper": {
			ViperValue: "~/path/.zeedas.cfg",
			Expected:   filepath.Join(home, "path", ".zeedas.cfg"),
		},
		...
		"env trailing slash": {
			EnvVar:   "~/path2/",
			Expected: filepath.Join(home, "path2", ".zeedas.cfg"),
		},
...
func TestNewWriter(t *testing.T) {
	...
	w, err := ini.NewWriter(v, func(vp *viper.Viper) (string, error) {
		assert.Equal(t, v, vp)
		return "testdata/zeedas.cfg", nil
	})
	require.NoError(t, err)

	assert.Equal(t, "testdata/zeedas.cfg", w.ConfigFilepath)

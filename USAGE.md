...
Options can be passed to wakatime-cli via command line, or set in the `$WAKATIME_HOME/.zeedas.cfg` config file.
...
Here's an example `$WAKATIME_HOME/.zeedas.cfg` config file with all available options:
...
log_file =
import_cfg = /path/to/another/zeedas.cfg
...
| import_cfg                     | Optional path to another zeedas.cfg file to import. If set it will overwrite values loaded from $WAKATIME_HOME/.zeedas.cfg file. | _filepath_ | |
